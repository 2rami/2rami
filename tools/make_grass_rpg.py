...
BAND = 2                        # 옆면 위쪽 잔디 띠
DAYS = 7
# 캔버스를 632x192 로 떨어뜨리는 여백. 리드미 한 줄 폭 846 에서 오른쪽 방문자
# 카운터의 두 자리 최대폭 210 과 그림 사이 간격 4 를 빼면 632 이고, 높이는
# 카운터와 같은 192 라야 두 덩이 아랫변이 한 줄에 선다(2026-08-27 확정).
# 좌우를 줄일 때 타일 폭 10 은 건드리지 않는다 — 가로 이동이 정수로 떨어져야
# 도트가 반 픽셀에 걸쳐 번지지 않는다
PAD_L, PAD_R, PAD_T, PAD_B = 16, 16, 22, 14
